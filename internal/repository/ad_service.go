...
type IAdService interface {
	...
	Create(ctx context.Context, ad model.AdService) (int, error)
...
func NewAdService(db *gorm.DB) IAdService {
	return &adService{db: db}
}

func (repo *adService) Get(ctx context.Context, f model.FilterAdService) ([]model.AdService, int, error) {
	res := make([]model.AdService, 0)
	...
	if f.ServiceBrandDetail != nil && *f.ServiceBrandDetail {
		stmt = stmt.Preload("ServiceBrand")
	}
	if f.ServiceSubcategoryDetail != nil && *f.ServiceSubcategoryDetail {
	...
	if len(f.ServiceСategoryIDs) != 0 {
		stmt = stmt.Where(`service_sub_category_id in (SELECT id
			...
			WHERE service_categories_id in (?))`, f.ServiceСategoryIDs)
	}
	if len(f.ServiceBrandIDs) != 0 {
		stmt = stmt.Where("service_brand_id in (?)", f.ServiceBrandIDs)
	}
	if f.Title != nil {
	...
	if f.HaveParamCondition {
		stmt2 := repo.db.WithContext(ctx).Table("ad_service_params").Select("ad_service_id")
		...
		rows, err := stmt2.Rows()
		if err != nil {
			return nil, 0, fmt.Errorf("repository adService `Get` `GetParam`: %w", stmt.Error)
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&id); err != nil {
				return nil, 0, fmt.Errorf("repository adService `Get` `GetParam` `Scan`: %w", stmt.Error)
			}
	...
	stmt = stmt.Find(&res)
	if stmt.Error != nil {
		return nil, 0, fmt.Errorf("repository adService `Get` `Find`: %w", stmt.Error)
	}
	...
	stmt = stmt.Count(&n)
	if stmt.Error != nil {
		return nil, 0, fmt.Errorf("repository adService `Get` `Count`: %w", stmt.Error)
	}

	return res, int(n), nil
}

func (repo *adService) GetByID(ctx context.Context, id int) (model.AdService, error) {
	res := model.AdService{}

	err := repo.db.WithContext(ctx).
		Unscoped().
		Preload("User").
		Preload("ServiceBrand").
		Preload("ServiceSubCategory").
		Preload("City").
		Preload("Documents").
		Find(&res, id).Error
	if err != nil {
		return res, fmt.Errorf("repository adService `GetByID` `Find`: %w", err)
	}

	return res, nil
}

func (repo *adService) Create(ctx context.Context, ad model.AdService) (int, error) {
	tx := repo.db.Begin()

	err := tx.Omit("Params", "count_rate", "rating").Create(&ad).Error
	if err != nil {
		tx.Rollback()
		return 0, fmt.Errorf("repository adService: Create: Create base: %w", err)
	}

	ad.Params.AdServiceID = ad.ID

	err = tx.Create(&ad.Params).Error
	if err != nil {
		tx.Rollback()
		return 0, fmt.Errorf("repository adService: Create: Create params: %w", err)
	}
...
func (repo *adService) Update(ctx context.Context, ad model.AdService) error {
	value := map[string]interface{}{
		"user_id":                 ad.UserID,
		"service_brand_id":        ad.ServiceBrandID,
	...
	err := repo.db.WithContext(ctx).Model(&model.AdService{}).Where("id = ?", ad.ID).Updates(value).Error
	if err != nil {
		return fmt.Errorf("repository adService `Update` `Updates`: %w", err)
	}

	return nil
}

func (repo *adService) Delete(ctx context.Context, id int) error {
	err := repo.db.WithContext(ctx).Delete(&model.AdService{}, id).Error
	if err != nil {
		return fmt.Errorf("repository adService `Delete` `Delete`: %w", err)
	}

	return nil
}

func (repo *adService) GetByIDSeen(ctx context.Context, id int) (int, error) {
	stmt := repo.db.WithContext(ctx).Table("ad_service_seen")

	rows := stmt.Where("ad_services_id = ?", id).Select("count").Row()
	if rows.Err() != nil {
		return 0, fmt.Errorf("repository adService `GetByIDSeen`: %w", rows.Err())
	}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return 0, fmt.Errorf("repository adService `GetByIDSeen`: %w", model.ErrNotFound)
		}
		return 0, fmt.Errorf("repository adService `GetByIDSeen`: %w", err)
	}

	return count, nil
}

func (repo *adService) CreateSeen(ctx context.Context, id int) error {
	stmt := repo.db.WithContext(ctx).Table("ad_service_seen")

	stmt = stmt.Exec(`INSERT INTO ad_service_seen (ad_services_id)
	VALUES (?)`, id)
	if stmt.Error != nil {
		return fmt.Errorf("repository adService `CreateSeen`: %w", stmt.Error)
	}

	return nil
}

func (repo *adService) IncrementSeen(ctx context.Context, id int) error {
	stmt := repo.db.WithContext(ctx).Table("ad_service_seen")

	stmt = stmt.Exec(`UPDATE ad_service_seen
	SET count = count + 1
	WHERE ad_services_id = ?`, id)
	if stmt.Error != nil {
		if errors.Is(stmt.Error, gorm.ErrRecordNotFound) {
			return fmt.Errorf("repository adService `IncrementSeen`: %w", model.ErrNotFound)
		}
		return fmt.Errorf("repository adService `IncrementSeen`: %w", stmt.Error)
	}

	return nil
}

func (repo *adService) GetCountBySubCategory(ctx context.Context) (map[int]int, error) {
	rows, err := repo.db.Raw(`
		SELECT service_sub_category_id, count(service_sub_category_id)
		FROM ad_services
		WHERE deleted_at IS NULL
		GROUP BY service_sub_category_id
		ORDER BY count(service_sub_category_id) DESC`).
		Rows()
	if err != nil {
		return nil, err
	}
