...
type AdEquipment struct {
	...
	EquipmentBrand         EquipmentBrand       `json:"equipment_brand"`
	EquipmentSubCategoryID int                  `json:"equipment_sub_category_id"`
	EquipmentSubCategory   EquipmentSubCategory `json:"equipment_sub_category"`
	...
	Params                 AdEquipmentParams    `json:"params"`
	Documents              []Document           `json:"document"                 gorm:"many2many:ad_equipment_documents"   swaggerignore:"true"`
	UrlDocument            []string             `json:"url_foto"                 gorm:"-"`
	CountRate              *int                 `json:"count_rate"`
	Rating                 *float64             `json:"rating"`
}

type FilterAdEquipment struct {
	UserDetail                 *bool
	EquipmentBrandDetail       *bool
	EquipmentSubcategoryDetail *bool
	CityDetail                 *bool
	...
	IDs                        []int
	UserIDs                    []int
	EquipmentBrandIDs          []int
	EquipmentSubСategoryIDs    []int
	EquipmentСategoryIDs       []int
	CityIDs                    []int
	Title                      *string
	Description                *string
	Price                      ParamMinMax
...
type AdEquipmentInteracted struct {
	ID            int            `json:"id"`
	CreatedAt     time.Time      `json:"created_at"`
	UpdatedAt     time.Time      `json:"updated_at"`
...
type AdEquipmentDocuments struct {
	AdEquipmentID int
	DocumentID    int
}
