...
type AdService struct {
	...
	ServiceBrand         ServiceBrand       `json:"service_brand"`
	...
	Params               AdServiceParams    `json:"params"`
	Documents            []Document         `json:"document"                 gorm:"many2many:ad_service_documents"   swaggerignore:"true"`
	UrlDocument          []string           `json:"url_foto"                 gorm:"-"`
	CountRate            *int               `json:"count_rate"`
	Rating               *float64           `json:"rating"`
}

type FilterAdService struct {
	UserDetail               *bool
	ServiceBrandDetail       *bool
	ServiceSubcategoryDetail *bool
	CityDetail               *bool
...
type AdServiceInteracted struct {
	ID          int            `json:"id"`
	CreatedAt   time.Time      `json:"created_at"`
	UpdatedAt   time.Time      `json:"updated_at"`
...
type AdServiceDocuments struct {
	AdServiceId int
	DocumentId  int
}
