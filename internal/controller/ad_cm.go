...
type adConstructionMaterial struct {
	adConstructionMaterialService service.IAdConstructionMaterial
}

func NewAdConstructionMaterial(r *gin.Engine, auth service.IAuthentication, adConstructionMaterialService service.IAdConstructionMaterial) {
	h := adConstructionMaterial{
		adConstructionMaterialService: adConstructionMaterialService,
	}

	ad := r.Group("construction_material/ad_construction_material")
...
// @Tags			AdConstructionMaterial (объявления оборудование)
// @Accept			mpfd
// @Produce		json
// @Param			base	body		controller.Create.adConstructionMaterialRequest	true	"тело объявления, не body а formData, значение в json"
// @Param			foto	formData	file									true	"фото объявления"
// @Success		200		{object}	map[string]interface{}
// @Failure		400		{object}	map[string]interface{}
// @Failure		404		{object}	map[string]interface{}
// @Failure		500		{object}	map[string]interface{}
// @Router			/construction_material/ad_construction_material [post]
func (h *adConstructionMaterial) Create(c *gin.Context) {
	type adConstructionMaterialRequest struct {
		ConstructionMaterialBrandID       int                                `json:"construction_material_brand_id"`
		ConstructionMaterialSubСategoryID int                                `json:"construction_material_sub_сategory_id"`
		...
		Params                            model.AdConstructionMaterialParams `json:"params"`
	}

	ad := model.AdConstructionMaterial{}
	...
	if len(mf.Value["base"]) != 0 {
		d := mf.Value["base"][0]
		adR := adConstructionMaterialRequest{}
		...
		{
			ad.ConstructionMaterialBrandID = adR.ConstructionMaterialBrandID
			ad.ConstructionMaterialSubCategoryID = adR.ConstructionMaterialSubСategoryID
	...
	err = h.adConstructionMaterialService.Create(c, ad)
	if err != nil {
...
// @Param		construction_material_brand_detail						query		bool	false	"при значении true дает полную информацию параметра"
...
// @Param		construction_material_brand_id							query		integer	false	"филтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		construction_material_subcategory_id					query		integer	false	"филтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		construction_material_category_id						query		integer	false	"филтры по категориям `можно передавать несколько раз`"
...
// @Failure	404											{object}	map[string]interface{}
// @Failure	500											{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material [get]
func (h *adConstructionMaterial) Get(c *gin.Context) {
	f := model.FilterAdConstructionMaterial{}
	...
	if val, ok := c.GetQuery("construction_material_brand_detail"); ok {
		v, err := strconv.ParseBool(val)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("construction_material_brand_detail: %w", err).Error()})
			return
		}
		f.ConstructionMaterialBrandDetail = &v
	}
	if val, ok := c.GetQuery("construction_material_subcategory_detail"); ok {
		...
		f.ConstructionMaterialSubcategoryDetail = &v
	}
	...
	if vals, ok := c.GetQueryArray("construction_material_brand_id"); ok {
		f.ConstructionMaterialBrandIDs = make([]int, 0, len(vals))
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("construction_material_brand_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
			f.ConstructionMaterialBrandIDs = append(f.ConstructionMaterialBrandIDs, id)
		}
	}
	if vals, ok := c.GetQueryArray("construction_material_subcategory_id"); ok {
		...
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("construction_material_subcategory_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
	...
	if vals, ok := c.GetQueryArray("construction_material_category_id"); ok {
		...
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("construction_material_category_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
	...
	res, total, err := h.adConstructionMaterialService.Get(c, f)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"ad_construction_materials": res, "total": total})
}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/{id} [get]
func (h *adConstructionMaterial) GetByID(c *gin.Context) {
	...
	res, err := h.adConstructionMaterialService.GetByID(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"ad_construction_material": res})
}

// Update
// @Summary		Update Ad
// @Description	Update an AdConstructionMaterial with new details
// @Tags			AdConstructionMaterial (объявления оборудование)
// @Accept			json
// @Produce		json
// @Param			id	path		integer	true	"ad construction material  ID"
// @Param			body	body		controller.Update.adConstructionMaterialUpdateRequest	true	"Update details"
// @Success		200	{object}	map[string]interface{}
// @Failure		400	{object}	map[string]interface{}
// @Failure		404	{object}	map[string]interface{}
// @Failure		500	{object}	map[string]interface{}
// @Router			/construction_material/ad_construction_material/{id} [put]
func (h *adConstructionMaterial) Update(c *gin.Context) {
	type adConstructionMaterialUpdateRequest struct {
		UserID                            int      `json:"user_id"`
		ConstructionMaterialBrandID       int      `json:"construction_material_brand_id"`
		ConstructionMaterialSubСategoryID int      `json:"construction_material_subcategory_id"`
		CityID                            int      `json:"city_id"`
		Price                             float64  `json:"price"`
		Title                             string   `json:"title"`
		Description                       string   `json:"description"`
	...
	adR := adConstructionMaterialUpdateRequest{}
	if err := c.ShouldBindJSON(&adR); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid input: %s", err.Error())})
		return
	}

	updateAdConstructionMaterial := model.AdConstructionMaterial{
		ID:                                id,
		UserID:                            adR.UserID,
		ConstructionMaterialBrandID:       adR.ConstructionMaterialBrandID,
		ConstructionMaterialSubCategoryID: adR.ConstructionMaterialSubСategoryID,
		CityID:                            adR.CityID,
		Price:                             adR.Price,
		Title:                             adR.Title,
	...
	err = h.adConstructionMaterialService.Update(c, updateAdConstructionMaterial)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/{id} [delete]
func (h *adConstructionMaterial) Delete(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("id: %w", err).Error()})
		return
	...
	if err := h.adConstructionMaterialService.Delete(c, id); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/favorite [get]
func (h *adConstructionMaterial) GetFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	fav, err := h.adConstructionMaterialService.GetFavorite(c, user.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/:id/favorite [post]
func (h *adConstructionMaterial) CreateFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	f := model.FavoriteAdConstructionMaterial{}
	...
	f.UserID = user.ID
	f.AdConstructionMaterialID = id

	if err := h.adConstructionMaterialService.CreateFavorite(c, f); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/:id/favorite [delete]
func (h *adConstructionMaterial) DeleteFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	if err := h.adConstructionMaterialService.DeleteFavorite(c, model.FavoriteAdConstructionMaterial{
		UserID:                   user.ID,
		AdConstructionMaterialID: id,
	}); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/:id/seen [get]
func (h *adConstructionMaterial) GetSeen(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
	...
	count, err := h.adConstructionMaterialService.GetSeen(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/construction_material/ad_construction_material/:id/seen [post]
func (h *adConstructionMaterial) IncrementSeen(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"parameter": "id", "error": err.Error()})
		return
	...
	err = h.adConstructionMaterialService.IncrementSeen(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
