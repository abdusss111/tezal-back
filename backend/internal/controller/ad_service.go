...
type adService struct {
	adServiceService service.IAdService
}

func NewAdService(r *gin.Engine, auth service.IAuthentication, adServiceService service.IAdService) {
	h := adService{
		adServiceService: adServiceService,
	}

	ad := r.Group("service/ad_service")
...
// @Tags			AdService (объявления услуг)
// @Accept			mpfd
// @Produce		json
// @Param			base	body		controller.Create.adServiceRequest	true	"тело объявления, не body а formData, значение в json"
// @Param			foto	formData	file									true	"фото объявления"
// @Success		200		{object}	map[string]interface{}
// @Failure		400		{object}	map[string]interface{}
// @Failure		404		{object}	map[string]interface{}
// @Failure		500		{object}	map[string]interface{}
// @Router			/service/ad_service [post]
func (h *adService) Create(c *gin.Context) {
	type adServiceRequest struct {
		ServiceBrandID       int                   `json:"service_brand_id"`
		ServiceSubСategoryID int                   `json:"service_sub_сategory_id"`
		...
		Params               model.AdServiceParams `json:"params"`
	}
	...
	if len(mf.Value["base"]) != 0 {
		...
		{
			ad.ServiceBrandID = adR.ServiceBrandID
			ad.ServiceSubCategoryID = adR.ServiceSubСategoryID
	...
	err = h.adServiceService.Create(c, ad)
	if err != nil {
...
// @Param		service_brand_detail						query		bool	false	"при значении true дает полную информацию параметра"
...
// @Param		id											query		integer	false	"фильтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		user_id										query		integer	false	"фильтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		service_brand_id							query		integer	false	"фильтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		service_subcategory_id					query		integer	false	"фильтры по идентификаторам параметров `можно передавать несколько раз`"
// @Param		service_category_id						query		integer	false	"фильтры по категориям `можно передавать несколько раз`"
// @Param		city_id										query		integer	false	"фильтры по идентификаторам параметров `можно передавать несколько раз`"
...
// @Failure	404											{object}	map[string]interface{}
// @Failure	500											{object}	map[string]interface{}
// @Router		/service/ad_service [get]
func (h *adService) Get(c *gin.Context) {
	f := model.FilterAdService{}
	...
	if val, ok := c.GetQuery("service_brand_detail"); ok {
		v, err := strconv.ParseBool(val)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("service_brand_detail: %w", err).Error()})
			return
		}
		f.ServiceBrandDetail = &v
	}
	...
	if vals, ok := c.GetQueryArray("service_brand_id"); ok {
		f.ServiceBrandIDs = make([]int, 0, len(vals))
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("service_brand_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
			f.ServiceBrandIDs = append(f.ServiceBrandIDs, id)
		}
	}
	if vals, ok := c.GetQueryArray("service_subcategory_id"); ok {
		...
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("service_subcategory_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
	...
	if vals, ok := c.GetQueryArray("service_category_id"); ok {
		...
		for _, v := range vals {
			id, err := strconv.Atoi(v)
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("service_category_ids: query values: %s: error:  %w", v, err).Error()})
				return
			}
	...
	res, total, err := h.adServiceService.Get(c, f)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"ad_services": res, "total": total})
}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/service/ad_service/{id} [get]
func (h *adService) GetByID(c *gin.Context) {
	...
	res, err := h.adServiceService.GetByID(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"ad_service": res})
}

// Update
// @Summary		Update AdService
// @Description	Update an AdService with new details
// @Tags			AdService (объявления услуг)
// @Accept			json
// @Produce		json
// @Param			id	path		integer	true	"ad service ID"
// @Param			body	body		controller.Update.adServiceUpdateRequest	true	"Update details"
// @Success		200	{object}	map[string]interface{}
// @Failure		400	{object}	map[string]interface{}
// @Failure		404	{object}	map[string]interface{}
// @Failure		500	{object}	map[string]interface{}
// @Router			/service/ad_service/{id} [put]
func (h *adService) Update(c *gin.Context) {
	type adServiceUpdateRequest struct {
		UserID               int      `json:"user_id"`
		ServiceBrandID       int      `json:"service_brand_id"`
		ServiceSubСategoryID int      `json:"service_subcategory_id"`
		CityID               int      `json:"city_id"`
		Price                float64  `json:"price"`
		Title                string   `json:"title"`
		Description          string   `json:"description"`
	...
	adR := adServiceUpdateRequest{}
	if err := c.ShouldBindJSON(&adR); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid input: %s", err.Error())})
		return
	}

	updateAdService := model.AdService{
		ID:                   id,
		UserID:               adR.UserID,
		ServiceBrandID:       adR.ServiceBrandID,
		ServiceSubCategoryID: adR.ServiceSubСategoryID,
		CityID:               adR.CityID,
		Price:                adR.Price,
		Title:                adR.Title,
	...
	err = h.adServiceService.Update(c, updateAdService)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/service/ad_service/{id} [delete]
func (h *adService) Delete(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("id: %w", err).Error()})
		return
	...
	if err := h.adServiceService.Delete(c, id); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/service/ad_service/favorite [get]
func (h *adService) GetFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	fav, err := h.adServiceService.GetFavorite(c, user.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/service/ad_service/:id/favorite [post]
func (h *adService) CreateFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	if err := h.adServiceService.CreateFavorite(c, f); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404				{object}	map[string]interface{}
// @Failure	500				{object}	map[string]interface{}
// @Router		/service/ad_service/:id/favorite [delete]
func (h *adService) DeleteFavorite(c *gin.Context) {
	user, err := GetUserFromGin(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	if err := h.adServiceService.DeleteFavorite(c, model.FavoriteAdService{
		UserID:      user.ID,
		AdServiceID: id,
	}); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/service/ad_service/:id/seen [get]
func (h *adService) GetSeen(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
	...
	count, err := h.adServiceService.GetSeen(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Failure	404	{object}	map[string]interface{}
// @Failure	500	{object}	map[string]interface{}
// @Router		/service/ad_service/:id/seen [post]
func (h *adService) IncrementSeen(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"parameter": "id", "error": err.Error()})
		return
	...
	err = h.adServiceService.IncrementSeen(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
